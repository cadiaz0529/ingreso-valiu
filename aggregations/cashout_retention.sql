...
(
    ...
    FROM cashouts
    WHERE `Created At` < DATE_SUB((SELECT MAX(`Created At`) FROM cashouts),INTERVAL 4 WEEK) -- Si los datos son actuales, cambiamos por DATE_SUB(CURDATE(),INTERVAL 4 WEEK)
) A
LEFT JOIN
(
    SELECT DISTINCT `User ID` AS user_id
